...
<script type="text/javascript">
	var count1 = 1;
	var rowsToColorSkema3 = [];
	var rowsToColor1Skema3 = [];
	function rowColorFormatterSkema3(cellValue, options, rowObject) {
		if (cellValue == "WIN")
			rowsToColorSkema3[rowsToColorSkema1.length] = options.rowId;
		else
			rowsToColor1Skema3[rowsToColor1Skema1.length] = options.rowId;
		return cellValue;
	}
	...
	$(function() {
		...
		$("#list_skema3").jqGrid(
				{
					url : '/manager/json/get-bets3.action?SkemaID='
							+ <s:property value="#session.skema3ID"/>,
					datatype : "json",
					colNames : [ 'Bet', 'Type', "", "" ],
					colModel : [ {
						...
						hidden : true,
						editable : false,
						formatter : rowColorFormatterSkema3
					}, {
						name : 'IDbet',
						index : 'IDbet',
						width : 50,
					...
					loadComplete : function() {
						...
						for (var i = 0; i < rowsToColorSkema3.length; i++) {

							$("#" + rowsToColorSkema3[i]).find("td").css({
								'background' : 'green'
							});

						}
						for (var i = 0; i < rowsToColor1Skema3.length; i++) {

							$("#" + rowsToColor1Skema3[i]).find("td").css({
								'background' : 'red'
							});
...
<body>
	...
	<s:if test="%{#session.skema2ID!=0}">
		<a href="/manager/userSkema2.action">Skema 2</a>
		<br>
	</s:if>
	...
	<div id="tabs">
		...
		<s:if test="%{#session.skema3ID!=0}">
			<div id="skema3">
				...
				Number of Events:
				<s:property value="#session.skema3.NEvents" />
				<br> Number of Expected Events:
				<s:property value="#session.skema3.NExpected" />
				<br> Number of Played Events:
				<s:property value="#session.skema3.played" />
				<br> Number of Successfull Events:
				<s:property value="#session.skema3.won" />
				<br> Initial Cash :
				<s:property value="#session.skema3.initialCash" />
				<br> Final Cash :
				<s:property value="#session.skema3.roundedFinalCash" />
				<br> Share :
				<s:property value="#session.skema3.share" />
				<br>

				<table id="list_skema3"></table>
				<div id="pager_skema3"></div>
			...
			<s:form action="request-bet">
						<s:hidden name="description" value="Skema 3" />
						<s:hidden name="initialAmount" value="%{nextBet3}" />
					...
					<s:form action="delete-skema">
						<s:hidden name="skemaID" value="%{#session.skema3ID}" />
