...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#list_skema1").jqGrid(
				{
					url : '/manager/json/get-bets1.action?SkemaID='
							+ <s:property value="#session.skema1ID"/>,
					datatype : "json",
					colNames : [ 'Bet', 'Type', "" , ""],
					colModel : [ {
					...
					loadComplete : function() {
						...
						for (var i = 0; i < rowsToColorSkema1.length; i++) {
							...
							$("#" + rowsToColorSkema1[i]).find("td").css({
								'background' : 'green'
							});
						...
						for (var i = 0; i < rowsToColor1Skema1.length; i++) {
							...
							$("#" + rowsToColor1Skema1[i]).find("td").css({
								'background' : 'red'
							});
	...
	<div id="tabs">
		...
		<s:if test="%{#session.skema1ID!=0}">
			<div id="skema1">
				...
				Number of Events:
				<s:property value="#session.skema1.NEvents" />
				<br> Number of Expected Events:
				<s:property value="#session.skema1.NExpected" />
				<br> Number of Played Events:
				<s:property value="#session.skema1.played" />
				<br> Number of Successfull Events:
				<s:property value="#session.skema1.won" />
				<br> Initial Cash :
				<s:property value="#session.skema1.initialCash" />
				<br> Final Cash :
				<s:property value="#session.skema1.roundedFinalCash" />
				<br> Share :
				<s:property value="#session.skema1.share" />
				<br>
			...
			<s:form action="request-bet">
					...
					<s:form action="delete-skema">
						<s:hidden name="skemaID" value="%{#session.skema1ID}" />
