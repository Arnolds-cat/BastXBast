		...
		$("#list_skema1").jqGrid(
				{
					...
					editurl : '/admin/json/edit-subbets.action',
					rowNum : 5,
					rowList : [ 5, 10, 15 ],
					pager : '#pager_skema1',
					sortname : 'subbetID',
...
<body>
	...
	<div id="tabs">
		...
		<div id="skema1">
			...
			<s:if test="%{waitForResult}">
				<s:form action="/admin/check">
					<s:hidden name="betIDch" value="%{IDbet}" />
					<s:select name="result" label="Result" list="#{0:'LOST', 1:'WON'}"
						emptyOption="false" />
					<s:submit value="Submit result" />
				</s:form>
			</s:if>
			...
			Evaluate Bet
			<s:form action="/admin/evaluate-bet">
				<s:hidden name="betIDev" value="%{IDbet}"/>
				<s:textfield id="newShare" name="newShare" readonly="true" label="Share"/>
				<s:submit value="Calculate"></s:submit>
        Earn: <s:property value="otherEarn" />
			</s:form>

			<s:form action="/admin/other-execute-bet">
				<s:hidden name="betIDev" value="%{IDbet}" />
				<s:hidden name="newShare" />
				<s:submit value="Confirm Execution" />
			</s:form>
