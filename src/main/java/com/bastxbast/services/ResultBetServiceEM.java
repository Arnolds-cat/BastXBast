...
public class ResultBetServiceEM implements ResultBetService {
	...
	public int getNumeroBets() {
		...
		int numero = ((Number) em.createQuery("select count(a.id) from Bet a WHERE a.state = :state").setParameter("state", Bet.BetState.WAITING_RESULT).getSingleResult()).intValue();
	...
	public Collection<Bet> getBets(int i, int rows, String sord, String sidx) {
		...
		query.setParameter("state", Bet.BetState.WAITING_RESULT);
