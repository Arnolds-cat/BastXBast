...
public class AddSkemaAction extends ActionSupport implements SessionAware {
	...
	public String execute() {
		...
		try {
			...
			User manager = (User)(map.get("user"));
			float income = initialCash*manager.getManagerPercent()/100.0f + manager.getIncome();
			map.put("income", income);
			skemaService.updateManagerIncome(initialCash, manager);
